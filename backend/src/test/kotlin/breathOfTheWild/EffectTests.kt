...
@KtorExperimentalAPI
class EffectTests : BaseTest() {
    private lateinit var image: Image

    private fun newItem(addition: Int, id: Int? = null) = Effect(
        id,
        addition.toString(),
        image,
        "descriptionHere",
    ...
    private val path = Path.BreathOfTheWild.effects
    ...
    fun `verify getting base url returns all items in table`() {
        ...
        val responseItems = request.response.content.parseResponse<EffectsResponse>()?.items
    ...
    fun `verify getting an added item`() {
        ...
        val request = get(path).send<Effect>()
        val addedItems = request.response.content.parseResponse<EffectsResponse>()?.items?.last()
